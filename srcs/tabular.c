...
/*   Updated: 2024/08/23 18:55:30 by tratanat         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
column_info *get_col_info(t_directory *directory)
{
    ...
    for (int i = 0; i < max_cols; i++)
    {
        ...
        infos[i].names = malloc(sizeof(char *) * file_count);
...
void print_tabular(t_directory *directory)
{
    ...
    int padding = 0;
    char *name = NULL;
    const int row_count = (col_info->file_count + (col_info->col_count - 1)) / col_info->col_count;
    int offset = 0;
    ...
    for (int row = 0; row < row_count; row++)
    {
        for (int col = 0; col < col_info->col_count; col++)
        {
            ...
            padding = col_info->cols[col] - strlen(name);
            printf("%s", name);
